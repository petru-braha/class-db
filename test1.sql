...
DECLARE
    ...
    CURSOR arr_prof IS SELECT * FROM profesori;
    it_prof arr_prof%ROWTYPE;
    
    v_id_prof profeori.id%TYPE := 0;
    CURSOR arr_curs IS SELECT * FROM cursuri c
    ...
    CURSOR arr_note IS SELECT * FROM note n 
...
BEGIN
    ...
    SELECT DISTINCT COUNT(*) INTO v_count_restantieri FROM profesori p
    JOIN didactic d ON p.id = d.id_profesor
    JOIN note n ON n.id_curs = d.id_curs
    JOIN studenti s ON s.id = n.id_student
        WHERE n.valoare IS NOT NULL AND n.valoare < 5;
    
    SELECT DISTINCT COUNT(*) INTO v_count_trecuti FROM profesori p
    JOIN didactic d ON p.id = d.id_profesor
    JOIN note n ON n.id_curs = d.id_curs
    JOIN studenti s ON s.id = n.id_student
    ...
    dbms_output.put_line(v_count_restantieri || ' ' || v_count_trecuti);
    dbms_output.put_line(v_procent);
    
    UPDATE profesori SET grad_didactic = grad_didactic || 3
        WHERE v_procent < 14.5;
    UPDATE profesori SET grad_didactic = grad_didactic || 2
        WHERE v_procent >= 14.6 AND v_procent < 16;
    UPDATE profesori SET grad_didactic = grad_didactic || 1
        WHERE v_procent >= 16;

/*
Atenție, dacă un profesor nu predă niciun curs, 
atunci afișați un mesaj corespunzător împreună cu 
id-ul, numele și prenumele acelui profesor.
Obligatoriu se va utiliza un cursor ce permite update-ul informațiilor.
*/

/*
Apoi creați un al doilea bloc anonim 
în care să citiți de la tastatură o valoare între 1 și 3 și
în funcție de această valoare afișați, 
câți profesori au adăugat la final de grad didactic acea valoare
și care sunt acei profesori cu id-ul lor,
numele și prenumele lor și gradul lor didactic.
